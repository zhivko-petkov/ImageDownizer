...
{
    internal class ConsequenceService
    {
        public void DownscaleImage(string imagePath, string savePath, string percDownSize)
        {
            ...
            using (Image<Rgba32> image = Image.Load<Rgba32>(imagePath))
            {
                ...
                var (redDownSizedSeparator, greenDownSizedSeparator, blueDownSizedSeparator) = DownSizeSeparatedColors(redSeparator, greenSeparator, blueSeparator, downSize);
                SaveDownSizedImage(redDownSizedSeparator, greenDownSizedSeparator, blueDownSizedSeparator, savePath);
            }

        }

        private static (byte[,], byte[,], byte[,]) ColorSeparate(Image<Rgba32> image)
        {
